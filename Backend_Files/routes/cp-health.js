...
function parseDay(dayStr) {
  const dt = DateTime.fromISO(dayStr, { zone: 'utc' });
  if (!dt.isValid) return null;
  return dt.toISODate(); // normalized YYYY-MM-DD
}

// make a timestamptz from a day + "HH:mm" (or accept full ISO)
function stampFromDayAndTime(dayISO, timeOrISO, tz) {
  // allow full ISO direct pass-through
  if (DateTime.fromISO(timeOrISO).isValid) {
    return DateTime.fromISO(timeOrISO).toUTC().toISO();
  }
  // otherwise combine "YYYY-MM-DD" + "HH:mm" in user's tz (fallback UTC)
  const zone = tz || 'UTC';
  const dt = DateTime.fromISO(`${dayISO}T${timeOrISO}`, { zone });
  if (!dt.isValid) return null;
  return dt.toUTC().toISO(); // store as UTC
}
...
/**
 * GET /api/health/fasting/today
 * Returns today's fasting row (by date) if present.
 * Optional ?tz=Europe/London to interpret "today".
 */
router.get('/fasting/today', requireLogin, async (req, res) => {
  const log = getReqLogger(req);
  try {
    ...
    const { rows } = await pool.query(
      ...
      [req.session.user.id, todayISO]
    );
    res.json(rows[0] || null);
  } catch (err) {
    log.error({ err }, 'GET /fasting/today failed');
    res.status(500).json({ error: 'server_error' });
  }
});

/**
 * GET /api/health/fasting?from=YYYY-MM-DD&to=YYYY-MM-DD
 * Returns a range (inclusive). Good for charts/weekly summary.
 */
router.get('/fasting', requireLogin, async (req, res) => {
  const log = getReqLogger(req);
  try {
    ...
    log.error({ err }, 'GET /fasting failed');
    res.status(500).json({ error: 'server_error' });
  }
});

/**
 * POST /api/health/fasting
 * Upsert by (user_id, day).
 * Body:
 * {
 *   day: "YYYY-MM-DD",          // required
 *   first: "HH:mm" | ISO,       // optional
 *   last:  "HH:mm" | ISO,       // optional
 *   tz?:   "Europe/London"      // optional; used when HH:mm provided
 * }
 */
router.post('/fasting', requireLogin, async (req, res) => {
  const log = getReqLogger(req);
  try {
    const userId = req.session.user.id;
    const { day, first, last, tz } = req.body || {};
    const dayISO = parseDay(String(day || ''));
    if (!dayISO) return res.status(400).json({ error: 'invalid day' });

    const firstTs = first ? stampFromDayAndTime(dayISO, String(first), tz) : null;
    const lastTs  = last  ? stampFromDayAndTime(dayISO, String(last), tz)  : null;

    if (!firstTs && !lastTs) {
      return res.status(400).json({ error: 'first or last is required' });
    }

    // Upsert (user_id, day)
    const { rows } = await pool.query(
      `INSERT INTO user_fasting_days (user_id, day, first_meal_at, last_meal_at, updated_at)
       VALUES ($1, $2, $3, $4, now())
       ON CONFLICT (user_id, day)
       DO UPDATE SET
         first_meal_at = COALESCE(EXCLUDED.first_meal_at, user_fasting_days.first_meal_at),
         last_meal_at  = COALESCE(EXCLUDED.last_meal_at,  user_fasting_days.last_meal_at),
         updated_at    = now()
       RETURNING id, day, first_meal_at, last_meal_at`,
      [userId, dayISO, firstTs, lastTs]
    );

    log.info({ day: dayISO }, 'Fasting upserted');
    res.json(rows[0]);
  } catch (err) {
    log.error({ err }, 'POST /fasting failed');
    res.status(500).json({ error: 'server_error' });
  }
});

/**
 * PATCH /api/health/fasting/:id
 * Body can include { first?: null|ISO|"HH:mm", last?: null|ISO|"HH:mm", tz? }
 */
router.patch('/fasting/:id', requireLogin, async (req, res) => {
  const log = getReqLogger(req);
  try {
    const userId = req.session.user.id;
    ...
    // Load current to get its day for HH:mm combination
    const cur = await pool.query(
      `SELECT id, day FROM user_fasting_days WHERE id = $1 AND user_id = $2`,
      [id, userId]
    );
    if (cur.rowCount === 0) return res.status(404).json({ error: 'not_found' });
    const dayISO = cur.rows[0].day; // YYYY-MM-DD

    const { first, last, tz } = req.body || {};
    const firstTs = first === null ? null : (first ? stampFromDayAndTime(dayISO, String(first), tz) : undefined);
    const lastTs  = last  === null ? null  : (last  ? stampFromDayAndTime(dayISO, String(last),  tz) : undefined);

    const { rows } = await pool.query(
      `UPDATE user_fasting_days
          SET first_meal_at = COALESCE($1, first_meal_at),
              last_meal_at  = COALESCE($2, last_meal_at),
              updated_at     = now()
        WHERE id = $3 AND user_id = $4
        RETURNING id, day, first_meal_at, last_meal_at`,
      [firstTs, lastTs, id, userId]
    );
    res.json(rows[0]);
  } catch (err) {
    log.error({ err }, 'PATCH /fasting/:id failed');
    res.status(500).json({ error: 'server_error' });
  }
});

/**
 * DELETE /api/health/fasting/:id
 */
router.delete('/fasting/:id', requireLogin, async (req, res) => {
  const log = getReqLogger(req);
  try {
    const id = Number(req.params.id);
    if (!Number.isFinite(id)) return res.status(400).json({ error: 'invalid id' });
    ...
    const { rowCount } = await pool.query(
      `DELETE FROM user_fasting_days WHERE id = $1 AND user_id = $2`,
      [id, req.session.user.id]
    );
    if (rowCount === 0) return res.status(404).json({ error: 'not_found' });
    res.status(204).send();
  } catch (err) {
    log.error({ err }, 'DELETE /fasting/:id failed');
    res.status(500).json({ error: 'server_error' });
  }
});
